...
class CustomTextEditState extends State<CustomTextEdit> with TextInputClient {
  ...
  void _openInputConnection() {
    ...
    if (hasInputConnection) {
      ...
      // opendray fork: enable IME features so system keyboards (e.g. Gboard)
      // expose voice input, suggestions and personalized learning. xterm
      // upstream disables these because traditional shells expect strict
      // per-byte input, but our use case is Claude TUI prompt entry where the
      // whole utterance can be delivered via onInsert just like a paste.
