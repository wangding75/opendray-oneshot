...
class _AgentTaskDetailScreenState extends ConsumerState<AgentTaskDetailScreen> {
                ...
                children: [
                  ...
                  if (state.task!.status == AgentTaskStatus.waitingInput)
                    _WaitingInputCard(
                      ...
                      onContinue: (value) =>
                          _runAction(() => controller.continueTask(value)),
                    ),
                  _Controls(
                    task: state.task!,
                    loading: state.actionLoading,
                  ...
                  const SizedBox(height: 16),
                  _SectionTitle(icon: Icons.history, title: strings('runs')),
                  const SizedBox(height: 8),
                  _RunSelector(
                    runs: state.runs,
                  ...
                  const SizedBox(height: 16),
                  _SectionTitle(icon: Icons.terminal, title: strings('events')),
                  const SizedBox(height: 8),
                  _OutputToolbar(
                    filter: _filter,
                  ...
                  const SizedBox(height: 8),
                  _OutputList(events: state.events, filter: _filter),
...
class _StatusTimeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final strings = AgentTasksStrings();
    final entries =
        <({DateTime at, IconData icon, String title, String detail})>[
          (
            at: task.createdAt,
            icon: Icons.add_task,
...
class _WaitingInputCard extends StatefulWidget {
  const _WaitingInputCard({required this.loading, required this.onContinue});
...
class _OutputToolbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SegmentedButton<_OutputFilter>(
      segments: [
        ButtonSegment(
          value: _OutputFilter.stdout,
          label: Text(strings('stdout')),
        ),
        ButtonSegment(
          value: _OutputFilter.stderr,
          label: Text(strings('stderr')),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    if (visible.isEmpty) {
      return Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Text(strings('noOutput')),
        ),
      );
    }
    final itemHeight = filter == _OutputFilter.raw ? 110.0 : 72.0;
    final height = (visible.length * itemHeight).clamp(120.0, 520.0);
    return Card(
      color: Colors.black,
          ...
          for (var index = 0; index < artifacts.length; index++) ...[
            ListTile(
              ...
              subtitle: Text(
                ...
                'SHA-256 ${artifacts[index].sha256.substring(0, 16)}…',
              ),
              isThreeLine: true,
              trailing: IconButton(
...
IconData _runIcon(AgentRunStatus status) => switch (status) {
  ...
  AgentRunStatus.collectingOutput => Icons.play_circle,
